...
from consul_host_fetcher import get_instances_from_consul
import logging
...
class TSocketPool(TSocket):
    ...
    def __init__(self, service_name, identity,timeout=None, conn_timeout=None, randomizer=None, max_total_timeout=None):
        ...
        self.max_total_timeout = max_total_timeout if max_total_timeout else 0.4
    ...
    def _define_metrics(self):
        ...
        metrics.define_counter("smart.socketpool.retry_num","num", prefix="abtest")
        metrics.define_timer("smart.socketpool.open_time","ms", prefix="abtest")
        metrics.define_counter("smart.socketpool.connect_fail","num", prefix="abtest")
        metrics.define_counter("smart.socketpool.acquire_fail","num", prefix="abtest")
        metrics.define_timer("smart.socketpool.nodesnum", "num", prefix="abtest")

    def open(self):
        metrics.emit_timer("smart.socketpool.nodesnum",len(self.servers), prefix="abtest", tagkv=self.tags)
        # Check if we want order randomization
        ts = time.time()
        if self._init or (ts - self.base_time) > random.randint(5, 10):
            self._init = False
            self.base_time = ts
            self.servers = get_instances_from_consul(self.service_name)
        ...
        for i in xrange(len(servers)):
            ...
            if lastFailtime == 0 or isLastServer or (lastFailtime > 0 and retryIntervalPassed):
                ...
                for attempt in xrange(self.numRetries):
                    try:
                        ...
                        metrics.emit_counter("smart.socketpool.retry_num",self.total_retry_num, prefix="abtest",tagkv=self.tags)
                        metrics.emit_timer("smart.socketpool.open_time", (time.time() - ts)*1000000, prefix='abtest',tagkv=self.tags)
                        # Successful connection, return now
                        return
                    except TTransportException as e:
                        # Connection failed
                        metrics.emit_counter("smart.socketpool.connect_fail", 1, prefix="abtest",tagkv=self.tags)
                        self.last_err = e

...
class Client(object):
    ...
    def __init__(
        ...
        Args:
            ...
            compress_response (bool): whether or not to gzip the GET response body. It's useful
    ...
    def get_lock(self, *args, **kwargs):
        return lock.Lock(self, *args, **kwargs)

    @property
    def election(self):
        return common.LeaderElection(self)

    def _result_from_response(self, response):
        """ Creates an EtcdResult from json dictionary """
        try:
            res = json.loads(response.data.decode('utf-8'))
            r = common.EtcdResult(**res)
    ...
    def _next_server(self):
        ...
        if machine_num == 0:
            raise Exception("server list of etcd is empty")
        self._base_uri_index = (self._base_uri_index + 1) % machine_num
        return machines[self._base_uri_index]
    ...
    def api_execute(self, path, method, params=None, timeout=None):
        ...
        headers = {}
        if method == self._MGET and self.compress_response:
            headers["Accept-Encoding"] = "gzip"
        retries = 0
        while not response:
            if retries >= self.max_retries:
                raise Exception("Retry too many times, MAX_RETRIES:%d" % self.max_retries)
            retries += 1
    ...
    def _handle_server_response(self, response):
        ...
        if response.status in [200, 201]:
            ...
            try:
                ...
                r = None
            if r:
                common.EtcdError.handle(**r)
            else:
                raise common.EtcdException(resp)

...
import json
import sys
from pyutil.etcd._etcd.client import Client
...
class BaseOperations(object):
    ...
    def get_client(self, url, ca_cert=None, cert=None):
        ...
        self.client = Client(host=h, port=int(p), protocol=parsed.scheme, allow_reconnect=False, ca_cert=ca_cert, cert=cert)
...
class Restorer(BaseOperations):
    ...
    def write(self, entry):
        if isinstance(entry['value'], unicode):
            entry['value'] = entry['value'].encode('utf-8')

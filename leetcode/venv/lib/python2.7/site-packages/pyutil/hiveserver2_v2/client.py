...
def init_metrics():
    metrics.init(CONF)
    metrics.define_counter("execute.total", 'query')
    metrics.define_counter("execute.success", 'query')
    metrics.define_counter("execute.fail", 'query')
    metrics.define_tagkv("service", ['hive', 'impala'])
    metrics.define_tagkv("mode", ['direct', 'sync', 'async'])
init_metrics()


def connect(service_name, host='', port=None, cluster='default', timeout=DEFAULT_TIMEOUT, default_db=None, username=None, password=None,
    ...
    if not host:
        conf = Conf('/opt/tiger/ss_conf/ss/hiveserver.conf', bypass_cache=True)
        clusters = conf.get_values("all_{}_clusters".format(service_name))
        if not clusters or cluster not in clusters:
            raise HiveServer2Exception("no {} cluster found for {}".format(service_name, cluster))
        # FIXME: failure detection may be necessary
        host, port = random.choice(conf.get_values("{}_{}_server".format(cluster, service_name))).split(':')
    if not host or not port:
        raise HiveServer2Exception("invalid {} server: {}:{}".format(service_name, host, port))
    metrics.define_tagkv("server", ["{}_{}".format(host.replace('.', '-'), port)])
    return HiveServer2Client(service_name, host, int(port), timeout, default_db, username, password,
...
class HiveServer2Client(object):
    ...
    def __init__(self, service_name, host, port, timeout=45, default_db=None, username=None, password=None,
                 impersonation_enabled=False, use_sasl=True, use_ldap=False, use_kerberos=False, principal=None,
                 querycache_rows=5000, query_timeout=0):
        ...
        self.hs2_protocol_version = {
            'hive': TProtocolVersion.HIVE_CLI_SERVICE_PROTOCOL_V9,
    ...
    def cursor(self, operation_handle=None, operation_string=None):
        cursor = HiveServer2Cursor(self, self._sentry_client, operation_handle, operation_string)
        if self.default_db:
    ...
    def fetch_result(self, operation_handle, orientation=TFetchOrientation.FETCH_NEXT, schema=None, max_rows=1000):
        ...
        rows = []
        if self.hs2_protocol_version == TProtocolVersion.HIVE_CLI_SERVICE_PROTOCOL_V9:

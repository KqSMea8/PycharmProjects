...
class TSocketPool(TSocket):
    ...
    def __init__(self, host, port, local_host, local_port, timeout=None, conn_timeout=None, randomizer=None,  max_total_timeout=None,
                 use_translate=False):
        ...
        if type(port) is int and type(host) is list:
            for i in xrange(0, len(host)):
                self.servers.append((host[i], port))
    ...
    def open(self):
        #first connect local host
        if self.open_local() == 0:
            return

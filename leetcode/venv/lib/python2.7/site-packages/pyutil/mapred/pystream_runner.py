...
class Runner(object):
    ...
    def run(self, kind, stdin=sys.stdin, stdout=sys.stdout):
        global mapper, combiner, reducer, final, update_jobconf, gather_counters
        if kind == "map":
            mapper, final, update_jobconf, gather_counters = self.job.run_mapper(stdin, stdout)
        elif kind == "combiner":
            combiner, final, update_jobconf, gather_counters = self.job.run_combiner(stdin, stdout)
        elif kind == "reduce":
            reducer, final, update_jobconf, gather_counters = self.job.run_reducer(stdin, stdout)
        else:

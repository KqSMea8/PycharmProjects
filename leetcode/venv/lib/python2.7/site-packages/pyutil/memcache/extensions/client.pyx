...
class MemcacheClient(object):
    ...
    def detect_cluster(memcache_servers):
        ...
        if MemcacheClient._servers_to_cluster_mapping is None:
            ...
            try:
                from pyutil.program.conf2 import ConfParse
                CONF_PATH = '/opt/tiger/ss_conf/ss/memcache.conf'
                conf = ConfParse(CONF_PATH, resolve_ref=False)
                for cluster in conf.get_all().keys():
                    servers = conf.get_values(cluster)
                    MemcacheClient._servers_to_cluster_mapping[unicode(servers)] = cluster
            except ImportError:
                pass

        search = unicode(memcache_servers)
        if search in MemcacheClient._servers_to_cluster_mapping:
            return MemcacheClient._servers_to_cluster_mapping[search]
        else:
            return None
    ...
    def __init__(self, memcache_servers, key_tmpl=None, timeout=900, client=None,
                 cpp_server_hash=True, hash_ring=False, prefix=None, use_proxy=False, socket_timeout=0.25,
                 stats_interval=None, failure_limit=5):
        '''
        ...
        if not client:
            self.client, self.reuse_client = MemcacheClient.get_or_create_client(
                    ...
                    socket_timeout=socket_timeout, failure_limit=failure_limit)
        else:

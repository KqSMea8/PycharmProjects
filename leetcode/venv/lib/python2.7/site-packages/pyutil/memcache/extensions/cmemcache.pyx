...
import re
import logging
from time import time as get_time # workaround of conficts with local variable in set_multi
try:
...
_FAILURE_LIMIT = 5 # number of retry before marking it dead
_DEAD_RETRY = 10  # number of seconds before retrying a dead server.
_SOCKET_TIMEOUT = 0.25  #  number of seconds before sockets timeout.
_CONNECT_TIMEOUT = 0.05 # we use smaller connect timeout
...
class Client(local):
    ...
    def __init__(self, servers, debug=0, pickleProtocol=0,
                 ...
                 logging=0, failure_limit=_FAILURE_LIMIT):
        """
    ...
    def _debuglog(self, str):
        if self.debug:
            sys.stderr.write("MemCached: %s\n" % str)

    def _log(self, str, level=logging.ERROR):
        if self.debug:
            sys.stderr.write("MemCached: %s\n" % str)
        if self.logging:
            logging.log(level, "Memcached: %s" % str)
    ...
    def delete_multi(self, keys, time=0, key_prefix=''):
        ...
        # TODO: precisely timeout control?
    ...
    def delete(self, key, time=0):
        ...
        try:
            ...
            self._debuglog('Delete expected DELETED or NOT_FOUND, got: %s'
                    % repr(line))
        except socket.error, msg:
            if isinstance(msg, tuple): msg = msg[1]
            server.mark_dead(msg)
    ...
    def set_multi(self, mapping, time=0, key_prefix='', min_compress_len=0):
        ...
        for server, keys in server_keys.iteritems():
            ...
            try:
                ...
                # though the result is undefined when timeout, we put all
                # keys into notstored for safe
                for key in keys:
                    if prefixed_to_orig_key[key] not in notstored:
        ...
        # we try to precisely control the timeout when receiving
        recv_start_time = get_time()

        for server, keys in server_keys.iteritems():
            try:
                time_remained = self.socket_timeout - (get_time() - recv_start_time)
                if time_remained <= 0:
                    server.settimeout(0) # non-blocking mode
                else:
                    server.settimeout(time_remained)
        ...
        def _unsafe_get():
            ...
            try:
                ...
                if not rkey:
                    return None
                if rkey != key:
                    self._log("Maybe stale data in socket.")
                    raise _Error("Maybe stale data in socket. "
                                 "expected: %s, actual: %s" % (key, rkey))
                try:
        ...
        for server in server_keys.iterkeys():
            try:
                ...
                while line and line != 'END':
                    ...
                    # Yes, when server return ERROR instead of VALUE or END
                    if rkey is None:
                        raise _Error("Unexpected VALUE. %s" % line)
                    val = self._recv_value(server, flags, rlen)
                    if rkey not in prefixed_to_orig_key:
                        self._log("Maybe stale data in socket.")
                        raise _Error("Maybe stale data in socket. "
                                     "%s not found in current mget" % rkey)
    ...
    def _recv_value(self, server, flags, rlen):
        ...
        if  flags == 0 or flags == Client._FLAG_COMPRESSED:
            ...
            try:
                ...
                self._log('Pickle error: %s\n' % e)
                return None
        else:
            self._log("unknown flags on get: %x\n" % flags)
...
class _Host(object):
    ...
    def mark_dead(self, reason):
        self.dead_count += 1
        if self.dead_count >= self.failure_limit:
            self._log("server: %s. %s. dead_count: %s. Marking dead." % (
                self, reason, self.dead_count))
            self.dead_until = time.time() + self.dead_retry
            self.dead_count = 0
            self.dead_total += 1
        else:
            self._log("server: %s. %s. dead_count: %s. Suspicious dead." % (
                self, reason, self.dead_count), logging.DEBUG)
    ...
    def settimeout(self, v):
        if self.socket and hasattr(self.socket, 'settimeout'):
            self.socket.settimeout(v)
    ...
    def _get_socket(self):
        ...
        # we use smaller connect timeout
        if hasattr(s, 'settimeout'): s.settimeout(_CONNECT_TIMEOUT)
        try:
            s.connect(self.address)
        except socket.timeout, msg:
            self.mark_dead("connect: %s, timeout: %r" % (msg, self.socket_timeout))
            return None
        except socket.error, msg:
            if isinstance(msg, tuple): msg = msg[1]
    ...
    def expect(self, text, raise_exception=False):
        ...
        if line != text:
            self._debuglog("while expecting '%s', got unexpected response '%s'"
                    % (text, line))
        return line
    ...
    def __str__(self):
        d = ''
        #TODO revert this? provide a new method name name()?
        #if self.dead_until:
            #d = " (dead until %d)" % self.dead_until

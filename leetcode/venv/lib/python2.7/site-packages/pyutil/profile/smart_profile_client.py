...
class ThriftProfileClient(threading.local):
    ...
    def __init__(self, service_name, timeout=0.1, conn_timeout=0.05, retries=3, nonblock=True):
        _transport = SmartSocketPool.TSocketPool(service_name, timeout, conn_timeout)
        if nonblock:
            transport = TTransport.TFramedTransport(_transport)
        else:
            transport = TTransport.TBufferedTransport(_transport)

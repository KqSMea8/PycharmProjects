...
_METRIC_PREFIX = "web.memcacheclient"

class MemcacheClient(object):
    version = 1

    def __init__(self, service, psm=None, servers=None, key_tmpl=None, timeout=60, enable_metrics=True, **kwargs):
        """
        """
        cluster, psm, servers, service = get_cluster_info(service, psm, servers)
        ...
        if enable_metrics:
            self.metrics = CacheMetrics(cluster, _METRIC_PREFIX, psm, service)
        else:
...
class PyutilMemcacheClient(object):
    ...
    def __init__(self, service, psm=None, servers=None, key_tmpl=None, timeout=60, enable_metrics=True, **kwargs):
        """
        """
        self.client = MemcacheClient(service, psm, servers, key_tmpl, timeout, enable_metrics, **kwargs)
        if self.client.key_tmpl == None:
            self.client.key_tmpl = self.client.cluster + '?%s'

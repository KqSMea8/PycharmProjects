...
class Converter(object):
    ...
    def parse_options(self, opt, twopass=None):
        ...
        if 'map_metadata' in opt:
            m = opt['map_metadata']
            if not type(m) == int:
                raise ConverterError('map_metadata needs to be int')
            else:
                format_options.extend(['-map_metadata', str(m)])
        # aggregate all options
        optlist = audio_options + video_options + subtitle_options + format_options
        if 'watermark' in opt['video']:
            optlist = ['-i', str(opt['video']['watermark'])] + optlist

...
class VideoCodec(BaseCodec):
    ...
    def _aspect_corrections(self, sw, sh, w, h, mode):
        ...
        if mode == 'keep':
            # source is taller, need to scale width to same ratio
            if target_aspect > aspect:  # target is taller
                w0 = int(h * aspect)
                if w0 % 2 == 1:
                    w0 += 1
                return w0, h, None
            else:  # source is wider, need to scale height to same ratio
                h0 = int(w / aspect)
                if h0 % 2 == 1:
                    h0 += 1
                return w, h0, None
    ...
    def parse_options(self, opt):
        ...
        if 'rotate' in safe:
            if safe['rotate'] == 90 or safe['rotate'] == 270:
                (safe['src_width'], safe['src_height']) = (safe['src_height'], safe['src_width'])
        ...
        if 'rotate' in safe:
            if safe['rotate'] == 270:
                if filters is None:
                    filters = 'transpose=1,transpose=1,transpose=1,'
                else:
                    filters = '%s %s' % (filters, 'transpose=1,transpose=1,transpose=1,')
            elif safe['rotate'] == 180:
                if filters is None:
                    filters = 'transpose=1,transpose=1,'
                else:
                    filters = '%s %s' % (filters, 'transpose=1,transpose=1,')
            elif safe['rotate'] == 90:
                if filters is None:
                    filters = 'transpose=1,'
                else:
                    filters = '%s %s' % (filters, 'transpose=1,')

        if w and h and 'filtergraph' in safe:
            if filters is None:
                filters = '%s' % ('scale=%dx%d[sc];' % (w,h))
            else:
                filters = '%s%s' % (filters, ('scale=%dx%d[sc];' % (w,h)))

            wm_ow = safe['wm_width']
            wm_oh = safe['wm_height']
            wm_w = None
            wm_h = None
            wm_aspect = (1.0 * wm_ow) / (1.0 * wm_oh)

            wm_w = (w * wm_ow)/720
            wm_h = wm_w/wm_aspect

            filters = '%s%s' %(filters, ('[1]scale=%dx%d[logo];[sc][logo]' %(wm_w, wm_h)))
            filters = '%s %s' % (filters, str(safe['filtergraph']))

        if filters is not None:
            filters = '%s%s' % ('[0:v]', filters)
        if filters:
            optlist.extend(['-filter_complex', filters])

...
def _http_call(the_url, method, authorization, opener=None, **kw):
    '''
    ...
    try:
        ...
        if opener:
            ...
            resp = urlopen(req)
        body = resp.read()
        if resp.info().getheader('Content-Encoding', None) == 'gzip':
...
def _http_get(url, authorization=None, opener=None, **kw):
    ...
    return _http_call(url, _HTTP_GET, authorization, opener, **kw)

def _http_post(url, authorization=None, opener=None, **kw):
    logging.info('POST %s' % url)
    return _http_call(url, _HTTP_POST, authorization, opener, **kw)

def _http_upload(url, authorization=None, opener=None, **kw):
    logging.info('MULTIPART POST %s' % url)
    return _http_call(url, _HTTP_UPLOAD, authorization, opener, **kw)
...
class HttpObject(object):
    ...
    def __getattr__(self, attr):
        def wrap(**kw):
            ...
            return _http_call('%s%s.json' % (self.client.api_url, attr.replace('__', '/')), self.method, self.client.access_token, opener=self.client.opener, **kw)
        return wrap
...
class APIClient(object):
    ...
    '''
    def __init__(self, app_key=None, app_secret=None, redirect_uri=None, response_type='code', domain='api.weibo.com', version='2', opener=None):
    ...
    def set_access_token(self, access_token, expires_in, uid=None):
    ...
    def get_token_info(self, **kw):
        return _http_post(self.auth_url + 'get_token_info', **kw)
    ...
    def me(self):
        return self.users__show(uid=self.uid)

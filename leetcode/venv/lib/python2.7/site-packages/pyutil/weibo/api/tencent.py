...
'''
Python client SDK for tencent weibo API using OAuth 1.0
'''
...
import time, hmac, uuid, base64, urllib, urllib2, hashlib, logging, binascii
...
def _http_call(the_url, method, return_json=True, opener=None, **kw):
    '''
...
class HttpObject(object):
    ...
    def __getattr__(self, attr):
        def wrap(**kw):
            return self.client._oauth_request(self.method, '%s/%s' % (self.client.api_url, attr.replace('__', '/')), **kw)
        return wrap

class APIClient(object):

    def __init__(self, app_key, app_secret, token=None, callback=None, domain='open.t.qq.com', opener=None):
        ...
        self.auth_url = 'http://%s//cgi-bin' % domain
        self.api_url = 'http://%s/api' % domain
        self.opener = opener
        self.get = HttpObject(self, _HTTP_GET)
        self.post = HttpObject(self, _HTTP_POST)

    def _oauth_request(self, method, the_url, **kw):
        params = dict( \
    ...
    def get_request_token(self):
        ...
        params = dict(oauth_callback=self.callback, \
                ...
                oauth_timestamp=str(int(time.time())), \
                oauth_version=_OAUTH_VERSION)
        url = '%s/oauth/request_token' % self.auth_url
        ...
        r = _http_call(url, _HTTP_GET, return_json=False, opener=self.opener, **params)
    ...
    def get_authorize_url(self, oauth_token):
        ...
        '''
        return '%s/oauth/authorize?oauth_token=%s' % (self.auth_url, oauth_token)
    ...
    def get_access_token(self):
        ...
        url = '%s/oauth/access_token' % self.auth_url
        bs = _generate_base_string('GET', url, **params)
        key = '%s&%s' % (self.app_secret, self.oauth_token_secret)
        oauth_signature = _generate_signature(key, bs)
        #authorization = self.__build_oauth_header(params, oauth_signature=oauth_signature)
        params['oauth_signature'] = oauth_signature
        r = _http_call(url, _HTTP_GET, params, return_json=False, opener=self.opener)
        kw = _parse_params(r, False)
        return OAuthToken(**kw)
...
def _generate_base_string(method, the_url, **params):
    '''
    ...
    plist.sort()
    return '%s&%s&%s' % (method, _quote(the_url), _quote('&'.join(['%s=%s' % (k, v) for k, v in plist])))
